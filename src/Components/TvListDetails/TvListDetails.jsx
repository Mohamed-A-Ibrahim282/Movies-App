...
import { allTvShowsReviews } from "../../ReduxStore/ReviewsSlice";

const TvListDetails = () => {
  const { id } = useParams();
  async function getTvShowDetails() {
    try {
      return await axios.get(`https://api.themoviedb.org/3/tv/${id}`, {
        headers: {
  ...
  const { data, isLoading } = useQuery("getTvShowDetails", getTvShowDetails);
    ...
    <>
          ...
          >
            <div className="detailsLayer w-100 h-100 p-3">
              <h4 className="text-white fw-bold mb-4">
                {data.data.name} ({data.data.first_air_date.slice(0, 4)})
              </h4>
              ...
              <div className="row mb-2">
                {data.data.genres.map((genres, index) => (
                  <div
                    key={index}
      ...
      <div className="container">
        <div className="d-flex align-items-center justify-content-between">
          ...
          <Link to={`/TvShowsReviews/${id}`} className="Link my-0 fw-semibold">
                ...
                <div className="row">
                  ...
                  <div className="col-md-10">
                    ...
                    >
                      ...
                      {reviewsData[0]?.author_details.rating}%
                    </div>
                  </div>
                </div>
                    ...
                    .map((movie, index) => (
                      <div key={index} className="col-6 col-sm-4 col-md-2">
                        <div
                          ...
                          onClick={() => {
                            navigate(`/tvListDetails/${movie.id}`);
